...
export default function page({}: Props) {
  ...
  if (isLoading) return <div className="p-6 md:p-10">Loading your dashboard...</div>;
  if (!dashboardData)
  ...
  const getIconStyle = (type: string) => {
    switch (type?.toLowerCase()) {
      case "leave": return "uil:calendar";
      case "remote": return "uil:suitcase";
      case "emergency": return "mingcute:warning-line";
      default: return "mynaui:box-solid";
    }
  };
      ...
      <div className="flex flex-col gap-6 md:gap-8 w-full">
        ...
        <div className="flex flex-col p-4 md:p-6 gap-4 bg-white shadow-sm rounded-2xl border border-gray-50">
          <div className="flex flex-row gap-3 items-center">
            <div className="p-2 md:p-3 bg-purple-100 rounded-lg text-purple-700">
              <Icon icon="entypo:line-graph" className="w-5 h-5 md:w-6 md:h-6" />
            </div>
            <h2 className="font-bold text-lg md:text-xl">This Month's Overview</h2>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <AttendanceSummary title="Present" value={summary.present.toString()} colorClass="text-green-600" />
            <AttendanceSummary title="Leave" value={summary.leave.toString()} colorClass="text-red-600" />
            <AttendanceSummary title="Remote" value={summary.remote.toString()} colorClass="text-blue-600" />
            <AttendanceSummary 
              title="Rate" 
              value={`${Math.round((summary.present / summary.totalWorkDays) * 100)}%`} 
              colorClass="text-purple-600" 
            />
          </div>
          <div className="flex flex-row justify-between items-center bg-gray-50 p-4 rounded-xl">
            <div>
              <p className="text-gray-500 text-xs md:text-sm uppercase font-semibold">Total Work Days</p>
              <h2 className="text-lg md:text-xl font-bold text-gray-800">{summary.totalWorkDays} days</h2>
            </div>
            <Link href="/my-attendance" className="bg-white px-4 py-2 rounded-lg border border-purple-200 text-purple-600 font-bold text-xs md:text-sm hover:bg-purple-50 transition-colors">
              View Details
            </Link>
          </div>
        </div>
        ...
        <div className="flex flex-col p-4 md:p-6 rounded-2xl bg-white shadow-sm border border-gray-50">
          <div className="flex flex-row justify-between items-center mb-4">
            ...
            <Link href="/my-requests" className="text-xs md:text-sm font-bold text-amber-600 hover:underline">View All</Link>
          </div>
          <div className="flex flex-col gap-3">
            {recentRequests?.map((req: any) => (
              <div key={req.id} className="flex flex-row justify-between items-center p-3 border border-gray-100 rounded-xl hover:bg-gray-50 transition-colors">
                <div className="flex flex-row items-center gap-3">
                  <div className={`p-2 rounded-lg ${getTypeStype(req.type)}`}>
                    <Icon icon={getIconStyle(req.type)} className="w-5 h-5 md:w-6 md:h-6" />
                  </div>
                  <div className="flex flex-col">
                    <h3 className="font-bold text-xs md:text-sm">{req.type}</h3>
                    <p className="text-[10px] md:text-xs text-gray-500">{new Date(req.created_at).toLocaleDateString()}</p>
                  </div>
                </div>
                <div className={`px-2 py-1 rounded-full capitalize font-semibold text-[10px] md:text-xs ${getStatusStyle(req.status)}`}>
                  {req.status}
                </div>
              </div>
            ))}
      ...
      <div className="flex flex-col gap-6 w-full lg:w-[400px] shrink-0">
        ...
        <div className="flex flex-col rounded-2xl bg-white shadow-sm border border-gray-50 overflow-hidden">
          <div className="bg-purple-100 p-4 md:p-6">
            <h2 className="text-lg md:text-xl font-bold text-purple-900">Quick Actions</h2>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-3 p-4 md:p-6">
            <QuickActionLink href="/submit-request?type=leave" color="red" icon="uil:calendar" label="Request Leave" />
            <QuickActionLink href="/submit-request?type=remote" color="blue" icon="uil:suitcase" label="Remote Work" />
            <QuickActionLink href="/submit-request?type=emergency" color="amber" icon="mingcute:warning-line" label="Emergency Request" />
            <QuickActionLink href="/submit-request?type=general" color="purple" icon="basil:document-outline" label="General Request" />
          </div>
        </div>
        ...
        <div className="flex flex-col rounded-2xl bg-white shadow-sm border border-gray-50 overflow-hidden">
          <div className="bg-blue-50 p-4 md:p-6">
            <h2 className="text-lg md:text-xl font-bold text-blue-900">My Information</h2>
          </div>
          <div className="flex flex-col gap-4 p-4 md:p-6">
            <InfoItem label="Employee ID" value={employee.employee_code || "--"} />
            <InfoItem label="Position" value={employee.position} />
            <InfoItem label="Department" value={employee.department_name} />
            <Link href="/my-profile" className="mt-2 p-3 bg-blue-600 text-white rounded-xl text-center text-sm font-bold hover:bg-blue-700 transition-colors shadow-md shadow-blue-100">
              View Full Profile
            </Link>
          </div>
        </div>
        ...
        <div className="flex flex-col rounded-2xl bg-white shadow-sm border border-gray-50 overflow-hidden">
          <div className="bg-green-50 p-4 md:p-6">
            <h2 className="text-lg md:text-xl font-bold text-green-900">Upcoming Schedule</h2>
          </div>
          <div className="p-4 md:p-6 flex flex-col gap-3">
            {upcomingLeave?.length > 0 ? (
              upcomingLeave.map((leave: any) => {
                ...
                return (
                  <div key={leave.id} className="flex flex-row gap-3 justify-between items-stretch">
                    <div className={`flex flex-col gap-1 p-3 rounded-xl flex-1 border ${isRemote ? "bg-blue-50 border-blue-200 text-blue-700" : "bg-red-50 border-red-200 text-red-700"}`}>
                      <h3 className="text-[10px] uppercase font-bold tracking-wider">{leave.type}</h3>
                    ...
                    <div className={`px-3 flex items-center justify-center text-xs font-black rounded-xl border-2 ${isRemote ? "bg-white border-blue-200 text-blue-700 font-medium" : "bg-white border-red-200 text-red-700"}`}>
              ...
              <p className="text-xs text-gray-400 italic text-center py-4">No upcoming leaves scheduled.</p>
            )}
          </div>
        </div>
      </div>
...
const QuickActionLink = ({ href, color, icon, label }: any) => {
  ...
  return (
    <Link href={href} className={`flex items-center gap-3 p-3 bg-white rounded-xl border-2 font-bold text-sm transition-all hover:text-white hover:shadow-md ${colors[color]}`}>
